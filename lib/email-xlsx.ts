...
export async function generateStockAlertXlsx(params: {
	...
	for (const item of params.outOfStock)
		slot(item.brand ?? 'Other').out.push(item);
	for (const item of params.lowStock)
		slot(item.brand ?? 'Other').low.push(item);
	...
	const overallRows = [
		...
		{
			metric: 'Total Attention',
			count: params.outOfStock.length + params.lowStock.length,
		},
		{
			metric: 'Report Generated',
			count: new Date().toLocaleString('en-CA'),
		},
	];
	...
	summary.addRow({});
	const brandHeader = summary.addRow({
		metric: 'Brand',
		count: 'Out of Stock',
	});
