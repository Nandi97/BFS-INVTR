...
const ORG_CONFIG: Record<
	ZenotiOrg,
	{ label: string; subdomain: string; envKey: string }
> = {
	bfs: {
		label: 'Beauty First Spa',
		subdomain: 'beautyfirstspa',
		envKey: 'ZENOTI_BFS_API_KEY',
	},
	bl: {
		label: 'Beauty Logix',
		subdomain: 'beautylogix',
		envKey: 'ZENOTI_BL_API_KEY',
	},
};
...
export async function fetchZenotiCenters(
	org: ZenotiOrg
): Promise<ZenotiCenter[]> {
	const data = await zenotiGet<{ centers?: ZenotiCenter[] }>(
		org,
		'/v1/centers'
	);
	return data.centers ?? [];
...
export function mapZenotiStatus(
	status: string
): 'RAISED' | 'UPDATED' | 'DELIVERED' | 'CANCELLED' {
