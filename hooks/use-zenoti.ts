...
export function useCreateFulfillment() {
  ...
  return useMutation({
    mutationFn: (orderId: string) =>
      axios.post(`/zenoti/orders/${orderId}/fulfillment`).then((r) => r.data),
...
export function useUpdateFulfillmentItem() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      axios
        .patch(`/zenoti/fulfillments/${fulfillmentId}/items/${itemId}`, data)
        .then((r) => r.data),
...
export function useAddWalkInItem() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      axios.post(`/zenoti/fulfillments/${fulfillmentId}/items`, data).then((r) => r.data),
    onSuccess: (_data, vars) => {
      qc.invalidateQueries({ queryKey: ["zenoti-order", vars.orderId] });
    },
  });
...
export function useSubmitFulfillment() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ fulfillmentId }: { fulfillmentId: string; orderId: string }) =>
      axios.post(`/zenoti/fulfillments/${fulfillmentId}/submit`).then((r) => r.data),
    onSuccess: (_data, vars) => {
      qc.invalidateQueries({ queryKey: ["zenoti-order", vars.orderId] });
      qc.invalidateQueries({ queryKey: ["zenoti-orders"] });
    },
