...
export function useQbDisconnect() {
	...
	return useMutation({
		mutationFn: () =>
			api.post('/integrations/quickbooks/disconnect').then((r) => r.data),
...
export function useSaveQbConfig() {
	...
	return useMutation({
		mutationFn: (config: Partial<QbConfig>) =>
			api
				.put('/integrations/quickbooks/config', config)
				.then((r) => r.data),
		onSuccess: () => qc.invalidateQueries({ queryKey: QB_CONFIG_KEY }),
	});
}
...
export function useQbSyncStock() {
	...
	return useMutation({
		mutationFn: ({
			...
			api
				.post<SyncResult>('/integrations/quickbooks/sync-stock', {
					rows,
					location,
				})
				.then((r) => r.data),
...
export function useQbSyncSales() {
	...
	return useMutation({
		mutationFn: (rows: Record<string, string>[]) =>
			api
				.post<SyncResult>('/integrations/quickbooks/sync-sales', {
					rows,
				})
				.then((r) => r.data),
		onSuccess: () => {
		...
		queryFn: () =>
			api
				.get<{
					file: string | null;
					dir: string;
				}>('/integrations/quickbooks/import-xls')
				.then((r) => r.data),
	});
}
...
export function useQbImportXls() {
	...
	return useMutation({
		mutationFn: (location: string) =>
			api
				.post<XlsImportResult>('/integrations/quickbooks/import-xls', {
					location,
				})
				.then((r) => r.data),
		onSuccess: () => {
...
export function useQbApiSyncStock() {
	...
	return useMutation({
		mutationFn: (location?: string) =>
			api
				.post<SyncResult>('/integrations/quickbooks/items', {
					location: location ?? 'BF Warehouse',
				})
				.then((r) => r.data),
		onSuccess: () => {
			qc.invalidateQueries({ queryKey: SYNC_LOGS_KEY });
...
export function useQbApiSyncSales() {
	...
	return useMutation({
		mutationFn: () =>
			api
				.post<
					SyncResult & { period?: { from: string; to: string } }
				>('/integrations/quickbooks/sync-sales-api')
				.then((r) => r.data),
		onSuccess: () => {
			qc.invalidateQueries({ queryKey: SYNC_LOGS_KEY });
			qc.invalidateQueries({ queryKey: ['sales'] });
...
export function useQbNameSync() {
	...
	return useMutation({
		mutationFn: () =>
			api
				.post<NameSyncResult>(
					'/integrations/quickbooks/items/sync-names'
				)
				.then((r) => r.data),
		onSuccess: () => {
			qc.invalidateQueries({ queryKey: SYNC_LOGS_KEY });
