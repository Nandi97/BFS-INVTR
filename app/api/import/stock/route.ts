...
export async function POST(req: NextRequest) {
	...
	for (let i = 0; i < rows.length; i++) {
		...
		if (!row.identifier || row.quantity == null || !row.location) {
			errors.push(
				`Row ${i + 1}: identifier, location, and quantity are required`
			);
			skipped++;
			continue;
		}
		...
		if (isNaN(qty) || qty < 0) {
			errors.push(
				`Row ${i + 1}: invalid quantity "${row.quantity}" — must be >= 0`
			);
			skipped++;
			continue;
		}
		...
		try {
			...
			if (!product) {
				errors.push(
					`Row ${i + 1}: product not found "${row.identifier}"`
				);
				skipped++;
				continue;
			}
			if (!location) {
				errors.push(
					`Row ${i + 1}: location not found "${row.location}"`
				);
				skipped++;
				continue;
			}
					...
					create: {
						...
						reorderPoint:
							row.reorderPoint != null
								...
								: 0,
						reorderQty:
							row.reorderQty != null ? Number(row.reorderQty) : 0,
						minQuantity:
							row.minQuantity != null
			...
			errors.push(
				`Row ${i + 1} ("${row.identifier}"): ${err instanceof Error ? err.message : String(err)}`
			);
			skipped++;
		}
	}
