...
export async function POST(req: NextRequest) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      ...
      const brandId    = row.brand    ? (await getOrCreateBrand(row.brand)).id    : undefined;
      const categoryId = row.category ? (await getOrCreateCategory(row.category)).id : undefined;
      ...
      if (existing) {
        ...
        if (row.supplier) {
          ...
          await prisma.productSupplier.upsert({
            where:  { productId_supplierId: { productId: existing.id, supplierId: supplier.id } },
            update: { ...(row.cost != null ? { cost: Number(row.cost), isPreferred: true } : {}) },
            create: { productId: existing.id, supplierId: supplier.id, cost: row.cost != null ? Number(row.cost) : null, isPreferred: true },
          });
        }
