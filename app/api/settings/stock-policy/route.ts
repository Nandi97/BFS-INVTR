...
export async function GET() {
	try {
		...
		return NextResponse.json(
			{ error: 'Failed to load stock policy' },
			{ status: 500 }
		);
	}
}
...
export async function PATCH(req: NextRequest) {
	try {
		...
		const {
			updates,
		}: { updates: { id: string; targetStockMonths: number }[] } = body;
		...
		for (const u of updates) {
			...
			if (!u.id || isNaN(months) || months < 1 || months > 36) {
				errors.push(
					`Invalid: id=${u.id} targetStockMonths=${u.targetStockMonths}`
				);
				continue;
			}
			try {
