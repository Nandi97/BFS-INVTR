...
export async function GET(req: NextRequest) {
	...
	if (typeGroup === 'in') {
		conditions.push(
			Prisma.sql`sm.type IN ('PURCHASE_RECEIPT','ADJUSTMENT_IN','TRANSFER_IN')`
		);
	} else if (typeGroup === 'out') {
		conditions.push(
			Prisma.sql`sm.type IN ('ADJUSTMENT_OUT','SALE','TRANSFER_OUT')`
		);
	} else {
		// Exclude snapshots from unfiltered view
		conditions.push(
			Prisma.sql`sm.type NOT IN ('RECONCILIATION','OPENING_STOCK')`
		);
	}
