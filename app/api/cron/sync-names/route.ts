...
export async function GET(req: NextRequest) {
  ...
  if (!process.env.CRON_SECRET || auth !== `Bearer ${process.env.CRON_SECRET}`) {
    console.warn("[cron/sync-names] unauthorized — check CRON_SECRET env var");
    return NextResponse.json({ error: "unauthorized" }, { status: 401 });
  }

  const base = process.env.NEXT_PUBLIC_APP_URL;
  if (!base || base.includes("localhost")) {
    console.error("[cron/sync-names] NEXT_PUBLIC_APP_URL is not set to a production URL:", base);
    return NextResponse.json({ error: "NEXT_PUBLIC_APP_URL misconfigured" }, { status: 500 });
  }

  console.log(`[cron/sync-names] starting — ${new Date().toISOString()} — base: ${base}`);
  ...
  try {
    ...
    console.log(`[cron/sync-names] complete — ${new Date().toISOString()}:`, JSON.stringify(result).slice(0, 300));
