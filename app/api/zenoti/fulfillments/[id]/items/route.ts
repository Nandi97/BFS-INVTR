...
export async function POST(
  ...
  const fulfillment = await prisma.bfsFulfillment.findUnique({ where: { id: fulfillmentId } });
  ...
  if (body.productCode) {
    const p = await prisma.product.findFirst({
      where: { OR: [{ barcode: body.productCode }, { sku: body.productCode }] },
      select: { id: true },
    });
    productId = p?.id ?? null;
  }
  ...
  const item = await prisma.bfsFulfillmentItem.create({
    data: {
      ...
      fulfilledRetailQty:     body.fulfilledRetailQty     ?? body.requestedRetailQty     ?? 0,
      fulfilledConsumableQty: body.fulfilledConsumableQty ?? body.requestedConsumableQty ?? 0,
