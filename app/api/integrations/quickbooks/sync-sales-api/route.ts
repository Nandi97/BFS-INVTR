...
function buildColMap(
	...
	for (let i = 0; i < columns.length; i++) {
		...
		if (i === 0) {
			slots.push({ index: i, dataType: 'name' });
			continue;
		}
		...
		if (parsed) {
			...
			slots.push({
				index: i,
				dataType: type === 'Money' ? 'amount' : 'qty',
				...parsed,
			});
			continue;
		}
...
export async function POST() {
	...
	try {
		...
		if (!res.ok) {
			...
			throw new Error(
				`QB Reports API: ${res.status} ${txt.slice(0, 300)}`
			);
		}
	...
	const accumulator = new Map<
		string,
		Map<MonthKey, { qty: number; amount: number }>
	>();
	...
	for (const colData of dataRows) {
		...
		if (!accumulator.has(product.id))
			accumulator.set(product.id, new Map());
