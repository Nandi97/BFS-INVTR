...
function isWide(row: FlatRow | WideRow): row is WideRow {
  return MONTH_COLS.some((m) => (row as unknown as Record<string, unknown>)[m] != null);
}
...
export async function POST(req: NextRequest) {
  ...
  for (let i = 0; i < rows.length; i++) {
    const row = rows[i];
    if (!row.itemName?.trim()) { skipped++; continue; }
    ...
    try {
      ...
      if (isWide(row)) {
        ...
        for (let m = 0; m < 12; m++) {
          ...
          const raw = (row as unknown as Record<string, unknown>)[colKey];
          if (raw == null || raw === "") continue;
          const revenue = parseFloat(String(raw).replace(/[$,]/g, ""));
          if (isNaN(revenue)) continue;

          await prisma.salesRecord.upsert({
            where:  { productId_year_month: { productId: product.id, year, month: m + 1 } },
            update: { revenue, source: "QB_EXPORT", updatedAt: new Date() },
            create: { productId: product.id, year, month: m + 1, quantity: 0, revenue, source: "QB_EXPORT" },
          });
          synced++;
        }
      } else {
        ...
        if (isNaN(month) || month < 1 || month > 12) {
          errors.push(`Row ${i + 1}: invalid month "${(row as FlatRow).month}"`);
          skipped++;
          continue;
        }

        const revenue  = parseFloat(String((row as FlatRow).revenue).replace(/[$,]/g, "")) || 0;
        const quantity = (row as FlatRow).quantity != null
          ? Math.round(parseFloat(String((row as FlatRow).quantity)))
          : 0;

        await prisma.salesRecord.upsert({
          where:  { productId_year_month: { productId: product.id, year, month } },
          update: { quantity, revenue, source: "QB_EXPORT", updatedAt: new Date() },
          create: { productId: product.id, year, month, quantity, revenue, source: "QB_EXPORT" },
        });
        synced++;
      }
    } catch (err: unknown) {
      errors.push(`Row ${i + 1} "${row.itemName}": ${err instanceof Error ? err.message : String(err)}`);
      skipped++;
    }
  }
  ...
  await prisma.$transaction([
    ...
    prisma.integrationConfig.upsert({
      ...
      create: { provider: "QUICKBOOKS", config: {}, isActive: true, lastSyncAt: startedAt },
    }),
  ]);

  return NextResponse.json({ synced, skipped, errors: errors.slice(0, 30), total: rows.length });
}
