...
function formatAddress(addr: QbVendor["BillAddr"]): string | null {
  if (!addr) return null;
  return [addr.Line1, addr.City, addr.CountrySubDivisionCode, addr.PostalCode, addr.Country]
    .filter(Boolean)
    .join(", ") || null;
}
...
export async function POST() {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(`QB Vendors API: ${res.status} ${txt.slice(0, 300)}`);
    }
  ...
  for (const v of vendors) {
    if (!v.Active) { skipped++; continue; }

    const name = (v.DisplayName || v.CompanyName || "").trim();
    if (!name) { skipped++; continue; }

    const contactName = [v.GivenName, v.FamilyName].filter(Boolean).join(" ") || null;
    ...
    if (existing) {
      ...
      if (!existing.contactName && contactName) patch.contactName = contactName;
      if (!existing.email       && email)       patch.email       = email;
      if (!existing.phone       && phone)       patch.phone       = phone;
      if (!existing.address     && address)     patch.address     = address;

      if (Object.keys(patch).length > 0) {
        await prisma.supplier.update({ where: { id: existing.id }, data: patch });
        updated++;
      } else {
        skipped++;
      }
      ...
      await prisma.supplier.create({
        data: { name, contactName, email, phone, address, isActive: true },
      });
      created++;
    }
  }
  ...
  return NextResponse.json({ created, updated, skipped, total: vendors.length });
}
