...
export async function POST(req: NextRequest) {
	...
	const fromDate: string =
		body.fromDate ?? twoYearsAgo.toISOString().slice(0, 10);
	...
	for (const invoice of invoices) {
		...
		for (const line of invoice.Line) {
			...
			qtyByItemId.set(
				itemId,
				(qtyByItemId.get(itemId) ?? 0) + detail.Qty
			);
		}
		...
		for (const [itemId, qty] of qtyByItemId) {
			...
			try {
				...
				errors.push(
					`INV-${invoice.DocNumber} / ${product.name}: ${err instanceof Error ? err.message : String(err)}`
				);
			}
		}
	}
