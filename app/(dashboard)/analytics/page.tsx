...
import { TrendingUp, Package, DollarSign, BarChart3, Upload } from "lucide-react";
...
import { SalesImportDialog }  from "@/components/analytics/sales-import-dialog";
...
export default function AnalyticsPage() {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          >
            ...
            <SelectContent>
              ...
              {(brands as { id: string; name: string }[] | undefined)?.map((b) => (
      ...
      {isLoading ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <KpiCard
            ...
            value={totalQty > 0 ? Math.round(totalQty / 12).toLocaleString() : "—"}
      ...
      <Card>
        <CardHeader className="flex flex-row items-start justify-between pb-2">
          <div>
            ...
            <CardDescription>Units sold and revenue per month</CardDescription>
          </div>
          <Tabs value={chartMode} onValueChange={(v) => setChartMode(v as ChartMode)}>
            <TabsList className="h-8">
              <TabsTrigger value="quantity" className="text-xs px-3">Units</TabsTrigger>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <MonthlySalesChart data={summary?.monthly ?? []} mode={chartMode} />
          )}
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Top Products — {year}</CardTitle>
          <CardDescription>Ranked by total units sold (top 15)</CardDescription>
        </CardHeader>
