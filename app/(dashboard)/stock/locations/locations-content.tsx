      ...
      <CardHeader className="flex flex-row items-start justify-between pb-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <CardDescription className="text-xs font-mono">{loc.code}</CardDescription>
          </div>
        </div>
        <DropdownMenu>
      ...
      <CardContent className="space-y-1.5">
        ...
        {loc._count !== undefined && (
          <p className="text-xs text-muted-foreground">
            {loc._count.inventory} product{loc._count.inventory !== 1 ? "s" : ""} in stock
          </p>
        )}
      </CardContent>
    </Card>
...
export function LocationsPageContent() {
  ...
  async function handleToggle(loc: Location) {
    try {
      ...
      toast.success(`${loc.name} ${loc.isActive ? "deactivated" : "activated"}`);
    } catch {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Manage warehouses, retail locations, and virtual stock pools
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed p-16 text-center">
          <Warehouse className="size-10 text-muted-foreground mb-4" />
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(o) => !o && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete this location. Locations with stock records cannot be deleted — deactivate instead.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
