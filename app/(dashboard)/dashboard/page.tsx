...
import { UrgentReorderTable } from "@/components/dashboard/urgent-reorder-table";
...
async function KpiCards() {
  ...
  const lowStock   = reorderRows.filter((r) => r.quantity > 0 && r.reorderPoint > 0 && r.quantity <= r.reorderPoint).length;
  const okStock    = reorderRows.filter((r) => r.reorderPoint === 0 || r.quantity > r.reorderPoint).length;
