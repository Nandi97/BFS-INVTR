...
- **ProductCombobox requires `useProductsMinimal()`** — do not use `useProducts({ limit: 1000 })` for product pickers. The regular endpoint caps at 100 products per page and has no way to fetch all 847 active products in one call. `?minimal=true` bypasses pagination entirely.
