...
| QB Invoice posting       | Confirm accounting team sign-off + QB customer names per store                                                                                          | `POST /api/zenoti/fulfillments/[id]/submit` emails PDF + XLSX packing list; QB posting is the next step                                                                                    |
...
| Restore email routing    | Currently all import emails go to `order@beautylogix.ca` for testing                                                                                    | Update `TYPE_META` in `lib/zenoti-email.ts`: COSTCO + INVERNESS → `accounting@beautyfirstspa.com`; confirm OTHER recipient with team                                                       |
...
- **Zenoti import emails: all to `order@beautylogix.ca`** — `TYPE_META` in `lib/zenoti-email.ts` routes all supplier types to `order@beautylogix.ca` for testing. Before go-live, update COSTCO and INVERNESS entries to `accounting@beautyfirstspa.com`.
