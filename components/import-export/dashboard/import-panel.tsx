...
export function ImportPanel({
	...
	const resultEntries = result
		? Object.entries(result).filter(
				([k]) => k !== 'errors' && typeof result[k] === 'number'
			)
		: [];
		...
		<div className="space-y-4">
			...
			<div className="bg-muted/20 space-y-3 rounded-lg border p-4">
				...
				<p className="text-muted-foreground text-xs">
					* required &nbsp;·&nbsp; Header row is auto-detected
			...
			{result ? (
				<div className="space-y-3">
					...
					{result.errors?.length > 0 && (
						<Alert variant="destructive">
							...
							<AlertDescription>
								<ul className="max-h-36 list-disc space-y-0.5 overflow-y-auto pl-4 text-xs">
				...
				<Tabs defaultValue="paste">
					<TabsList className="h-8">
						<TabsTrigger value="paste" className="px-3 text-xs">
							Paste CSV
						</TabsTrigger>
						<TabsTrigger value="upload" className="px-3 text-xs">
							Upload File
						</TabsTrigger>
					</TabsList>
					...
					<TabsContent value="upload" className="mt-3 space-y-3">
						<div
							className="hover:bg-muted/30 flex cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed p-10 transition-colors"
