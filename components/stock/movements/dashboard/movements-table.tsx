...
function ExplainSheet({
	...
	async function onSubmit(values: ExplainForm) {
		...
		try {
			...
			toast.success(
				'Movement explained and removed from shrinkage queue'
			);
			form.reset();
			onOpenChange(false);
		} catch {
					...
					{isLoading ? (
						...
						rows.map((r) => (
							<TableRow key={r.productId}>
								...
								<TableCell className="text-muted-foreground text-right text-sm">
									{r.movementCount > 0
										? (r.totalIn / r.movementCount).toFixed(
												1
											)
										: '—'}
								</TableCell>
...
export function MovementsTable({
	...
	const { data: shrinkageCount } = useStockMovements({
		shrinkage: true,
		limit: 1,
	});
	...
	return (
		<div className="space-y-4">
			...
			<div className="flex flex-wrap items-center gap-2">
				{(
					['all', 'restocks', 'dispatches', 'shrinkage'] as Preset[]
				).map((p) => (
					<Button
						key={p}
					...
					>
						...
						{p === 'shrinkage' &&
							(shrinkageCount?.total ?? 0) > 0 && (
								<span
									className={cn(
										'ml-1.5 rounded-full px-1.5 py-0 text-[10px] leading-4 font-semibold',
										preset === 'shrinkage'
			...
			{isShrinkage && (
				<div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800 dark:border-amber-900/40 dark:bg-amber-950/20 dark:text-amber-300">
					<strong>Unexplained shrinkage</strong> — these are stock
					drops detected by the QB sync with no invoice attribution.
					Use "Explain" to record the reason and remove them from this
					queue.
				</div>
			)}
			...
			<div className="flex flex-wrap items-center gap-2">
				...
				{view === 'movements' && !isShrinkage && (
					...
					>
						...
						<SelectContent>
							...
							{Object.entries(MOVEMENT_LABEL).map(
								([value, label]) => (
				...
				{!isShrinkage && (
					<div className="ml-auto flex items-center gap-0.5 rounded-md border p-0.5">
						...
						<Button
							variant={
								view === 'by-product' ? 'secondary' : 'ghost'
							}
							size="sm"
							className="h-7 px-2.5 text-xs"
			...
			{showSummary && !isShrinkage && (
				<div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
					...
					<div className="flex items-center gap-3 rounded-lg border bg-red-50 p-4 dark:bg-red-950/20">
					...
					<div className="flex justify-end">
						<Button
							...
							onClick={() => {
								...
								if (sharedFilters.locationId)
									p.set(
										'locationId',
										sharedFilters.locationId
									);
								if (sharedFilters.brandId)
									p.set('brandId', sharedFilters.brandId);
								if (sharedFilters.dateFrom)
									p.set('dateFrom', sharedFilters.dateFrom);
								if (sharedFilters.dateTo)
									p.set('dateTo', sharedFilters.dateTo);
								window.open(
									`/api/stock/movements/summary/export?${p}`,
									'_blank'
								);
							}}
						>
						...
						<Table>
							...
							<TableBody>
								{isLoading ? (
									...
									rows.map((m) => {
										const isOut = OUT_TYPES.includes(
											m.type
										);
										return (
											<TableRow key={m.id}>
