...
function UrgencyBadge({ urgency }: { urgency: ReorderUrgency }) {
	...
	return (
		<Badge
			variant="outline"
			className={cn('text-xs whitespace-nowrap', cfg.className)}
		>
			{cfg.label}
		</Badge>
	);
...
export function ReorderTable({
	...
	const urgentCount = rows.filter(
		(r) => r.urgency === 'out' || r.urgency === 'urgent'
	).length;
					...
					<button
						...
						className={cn(
							...
							urgency === 'low'
								...
								: 'border-amber-500/40 text-amber-600 hover:border-amber-500 dark:text-amber-400'
						)}
					>
						Low Stock · {lowCount}
					</button>
							...
							<TableHead className="text-right">
								<div className="flex items-center justify-end gap-1">
									...
									<Tooltip>
										...
										<TooltipContent>
											Average monthly consumption from
											sales records
										</TooltipContent>
									</Tooltip>
								</div>
							...
							<TableRow>
								...
								>
									<div className="text-muted-foreground flex flex-col items-center gap-2">
										<CheckCircle2 className="mb-1 size-8 text-emerald-500" />
										<span className="text-sm font-medium">
											All stock levels are healthy
										</span>
										<span className="text-xs">
											No products are below their reorder
											point right now.
										</span>
									</div>
								</TableCell>
