...
export function MovementsReport() {
  ...
  function onExport() {
    exportCsv(
      ...
      ["Date", "Product", "SKU", "Brand", "Location", "Type", "Quantity", "Balance After", "Reference", "Notes"],
      rows.map((r) => [
        format(new Date(r.createdAt), "yyyy-MM-dd HH:mm"),
        r.productName, r.sku, r.brand, r.location,
        r.type, r.quantity, r.balanceAfter, r.reference, r.notes,
      ])
    );
  }
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>All stock movements in the selected date range (max 2,000 rows)</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        {!isLoading && data && (
          <div className="flex gap-6 rounded-lg border bg-muted/30 px-4 py-3">
            <div>
              <p className="text-xs text-muted-foreground">Units In</p>
              <p className="text-lg font-semibold text-green-600 dark:text-green-400">+{data.totalIn.toLocaleString()}</p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground">Units Out</p>
              <p className="text-lg font-semibold text-destructive">−{data.totalOut.toLocaleString()}</p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground">Net</p>
              <p className="text-lg font-semibold">{(data.totalIn - data.totalOut).toLocaleString()}</p>
            </div>
          </div>
        )}
        ...
        {isLoading ? (
          ...
          <div className="rounded-md border overflow-auto max-h-[520px]">
            <Table>
              ...
              <TableBody>
                {rows.length === 0 ? (
                  <TableRow><TableCell colSpan={8} className="text-center text-muted-foreground py-10">No movements in this range</TableCell></TableRow>
                ) : (
                  rows.map((r) => (
                    <TableRow key={r.id}>
                      ...
                      <TableCell>
                        <div className="font-medium leading-tight text-sm">{r.productName}</div>
                      ...
                      <TableCell>
                        <Badge variant={TYPE_VARIANT[r.type] ?? "outline"} className="text-xs whitespace-nowrap">
                          {r.type.replace(/_/g, " ")}
                        </Badge>
                      </TableCell>
                      <TableCell className="text-right tabular-nums font-medium">{r.quantity}</TableCell>
                      <TableCell className="text-right tabular-nums text-muted-foreground">{r.balanceAfter}</TableCell>
