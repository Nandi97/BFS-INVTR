...
export function StockHealthChart() {
  ...
  const counts = React.useMemo(() => ({
    ...
    urgent: rows.filter((r) => r.quantity > 0 && r.reorderPoint > 0 && r.quantity <= r.reorderPoint).length,
    low:    rows.filter((r) => r.quantity > 0 && r.reorderPoint > 0 && r.quantity > r.reorderPoint && r.quantity <= r.reorderPoint * 2).length,
    ok:     rows.filter((r) => r.quantity > 0 && (r.reorderPoint === 0 || r.quantity > r.reorderPoint * 2)).length,
  }), [rows]);
        ...
        {isLoading ? (
          ...
          <>
            ...
            >
              <PieChart>
                <defs>
                  {(["out","urgent","low","ok"] as const).map((key, i) => (
                    <linearGradient key={key} id={`fill-${key}`} x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%"   stopColor={`var(--color-${key})`} stopOpacity={1 - i * 0.05} />
                      <stop offset="100%" stopColor={`var(--color-${key})`} stopOpacity={0.85 - i * 0.05} />
                    </linearGradient>
                  ))}
                </defs>
                <ChartTooltip cursor={false} content={<ChartTooltipContent hideLabel />} />
                <Pie
                  data={chartData.map((d) => ({ ...d, fill: `url(#fill-${d.status})` }))}
                  dataKey="count"
                  nameKey="status"
                  innerRadius={56}
                  strokeWidth={2}
            ...
            <div className="space-y-1.5 mt-1">
              {(["out","urgent","low","ok"] as const).map((key) => {
                const n   = counts[key];
                ...
                return (
                  <div key={key} className="flex items-center justify-between text-sm">
                    ...
                    <div className="flex items-center gap-2 tabular-nums">
                      ...
                      <span className="text-muted-foreground text-xs w-7 text-right">{pct}%</span>
                    </div>
                  </div>
                );
              })}
