		...
		return (
			<div className="grid gap-3 sm:grid-cols-2">
				{Array.from({ length: 3 }).map((_, i) => (
					<div
						key={i}
						className="bg-muted/30 h-32 animate-pulse rounded-xl border"
					/>
				))}
			</div>
		);
	...
	if (rules.length === 0) {
		return (
			<div className="flex flex-col items-center justify-center rounded-xl border border-dashed p-12 text-center">
				<Bell className="text-muted-foreground mb-4 size-10" />
				<p className="text-sm font-medium">No alert rules configured</p>
				<p className="text-muted-foreground mt-1 max-w-xs text-xs">
					Add your first rule to start receiving email notifications
					about stock levels.
				</p>
	...
	return (
		<>
			<div className="grid gap-3 sm:grid-cols-2">
				{rules.map((rule) => {
					const cfg =
						TYPE_CONFIG[rule.type] ?? TYPE_CONFIG.DAILY_DIGEST;
					const Icon = cfg.icon;
					return (
						<Card
							key={rule.id}
							...
							<CardContent className="space-y-1.5">
								...
								{!rule.lastTriggeredAt && (
									<p className="text-muted-foreground flex items-center gap-1 text-xs">
										<BellOff className="size-3" /> Never
										triggered
									</p>
								)}
							</CardContent>
