 ...
 */
function parseQbSalesCsv(raw: string, defaultYear: number): Record<string, string>[] {
  ...
  const HEADER_MAP: Record<string, string> = {
    itemname: "itemName", name: "itemName", item: "itemName",
    month: "month", revenue: "revenue", amount: "revenue",
    qty: "quantity", quantity: "quantity", units: "quantity",
    year: "year", sku: "sku", barcode: "sku",
  ...
  if (isHeader) {
    ...
    colMap    = { 0: "itemName", 1: "month", 2: "year", 3: "quantity", 4: "revenue" };
    dataLines = lines;
  }
  ...
  return dataLines
    .map((line) => {
      ...
      if (row.month) {
        const mn = MONTH_NAMES.indexOf(row.month.toLowerCase().slice(0, 3));
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-1.5 max-w-[160px]">
        ...
        />
        <p className="text-xs text-muted-foreground">Used when year column is absent.</p>
      </div>

      <div className="space-y-1.5">
        <Label htmlFor="qb-sales-csv">Paste QB Sales by Product/Service Summary CSV</Label>
        <p className="text-xs text-muted-foreground">
          Supports two QB export formats:
          <br />
          <strong>Wide:</strong> Item Name, Jan, Feb, …, Dec (revenue per month)
          <br />
          <strong>Flat:</strong> Item Name, Month, Year, Quantity, Revenue
        </p>
        <Textarea
          id="qb-sales-csv"
          className="font-mono text-xs min-h-[200px]"
          placeholder={"Item Name,Jan,Feb,Mar,Apr,May,Jun,Jul,Aug,Sep,Oct,Nov,Dec\nTROIAREUKE RX 35ml,320.00,240.00,400.00,,,,,,,,,,\nBanhada Cleanser,180.00,150.00,,,,,,,,,,,"}
          value={csv}
      ...
      {result && (
        <div className={cn(
          ...
          <p>
            <span className="font-medium">{result.synced}</span> records synced,{" "}
