				...
				<TabsContent value="stock" className="mt-4 space-y-4">
							...
							<div className="flex items-center justify-between">
								<div>
									...
									<CardDescription className="mt-1">
										Pull live stock quantities directly from
						...
						<CardHeader>
							<div className="flex items-start justify-between gap-4">
								<div>
									...
									<CardDescription className="mt-1">
										Overwrites product names in BFS with the
										canonical names from QuickBooks, matched
										by SKU. Runs automatically on the 1st of
										each month. Use the button to run on
										demand after QB catalogue changes. Items
										without a QB SKU are skipped to avoid
										ambiguous matches.
									</CardDescription>
								</div>
							</div>
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Upload a <code>ProductServiceList__*.xls</code>{' '}
								export from QuickBooks to update stock
								quantities.
							</CardDescription>
						</CardHeader>
						<CardContent>
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Paste a CSV export from QuickBooks to update
								stock quantities. Negative quantities are
								clamped to 0 and a reconciliation movement is
								recorded.
							</CardDescription>
						</CardHeader>
						<CardContent>
				...
				<TabsContent value="sales" className="mt-4 space-y-4">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Paste a CSV export from QuickBooks to import
								monthly sales history. Supports both the wide
								(Jan–Dec columns) and flat (one row per month)
								formats.
							</CardDescription>
						</CardHeader>
						<CardContent>
						...
						<CardHeader>
							<div className="flex items-center justify-between">
								<div>
									...
									<CardDescription className="mt-1">
										Imports all active QB vendors into your
										Suppliers list. New vendors are created;
										existing suppliers are updated only if
										contact fields (email, phone, address)
										are currently blank. Manual edits are
										never overwritten.
									</CardDescription>
								</div>
								<Suspense fallback={null}>
				...
				<TabsContent value="config" className="mt-4 space-y-4">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Authorise this app to access your QuickBooks
								data via OAuth 2.0.
							</CardDescription>
						</CardHeader>
						<CardContent>
				...
				<TabsContent value="history" className="mt-4">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								All QuickBooks sync operations, newest first.
							</CardDescription>
						</CardHeader>
						<CardContent>
