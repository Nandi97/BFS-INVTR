...
import { useQbConfig, useQbApiSyncStock, useQbApiSyncSales } from "@/hooks/use-integrations";
...
export function QbApiSync({ mode }: Props) {
  ...
  if (mode === "stock") {
    return (
      ...
      >
        <RefreshCw className={`size-3.5 mr-1.5 ${syncStock.isPending ? "animate-spin" : ""}`} />
  ...
  return (
    <Button
      ...
      onClick={() =>
        syncSales.mutate(undefined, {
          onSuccess: (r) => {
            const period = r.period ? ` (${r.period.from} – ${r.period.to})` : "";
    ...
    >
      <RefreshCw className={`size-3.5 mr-1.5 ${syncSales.isPending ? "animate-spin" : ""}`} />
