...
function LeadTypeBadge({ days }: { days: number }) {
	return days <= LOCAL_THRESHOLD ? (
		...
		<Badge
			variant="outline"
			className="border-blue-500/40 text-xs text-blue-700 dark:text-blue-400"
		>
			International
		</Badge>
	);
...
function EditableRow({ brand }: { brand: Brand }) {
	...
	const save = useCallback(() => {
		if (leadDays === brand.leadTimeDays) {
			setEditing(false);
			return;
		}
		mutate(
...
export function BrandLeadTimesTable() {
	...
	const local = sorted.filter(
		(b) => b.leadTimeDays <= LOCAL_THRESHOLD
	).length;
	...
	return (
		<div className="space-y-4">
			...
			<p className="text-muted-foreground text-xs">
				Lead time affects the <strong>Reorder Point</strong> (how much
